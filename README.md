...
- A migration `0004` cria a tabela `direct_messages` (mensagens diretas entre amigos) com índices compostos em `(sender_id, recipient_id)` e `(recipient_id, sender_id)` para buscar rápido o histórico de uma conversa. **Ao atualizar um banco já existente, rode `alembic upgrade head`** (ou suba a API, que o faz sozinha).
